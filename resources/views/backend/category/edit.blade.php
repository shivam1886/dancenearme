...
@section('title') {{__('Update Dance')}} - {{$category->title}} @endsection

@section('content')

<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">{{__('Update Dance')}} <button class="btn btn-danger btn-dlt">{{__('Delete')}}</button></h1>
    </div>
</div>
<br/>
        <div class="row">
    ...
    <div class="modal-dialog modal-md">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">{{__('Delete Dance')}}</h4>
        </div>
        <div class="modal-body">
                @csrf
                {{ method_field('DELETE') }}
                 <input type="hidden" name="id" value="{{$category->id}}">
                 <p>{{__('Are you sure want to delete dance?')}}</p>
        </div>
